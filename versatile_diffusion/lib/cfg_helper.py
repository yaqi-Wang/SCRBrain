...
class model_cfg_bank(object):
    def __init__(self):
        # self.cfg_dir = osp.join('/home/data/wangyaqi/projects/10StableDiffusionReconstruction/10StableDiffusionReconstruction-main/', 'versatile_diffusion/configs/model')
...
def get_command_line_args():
    ...
    parser.add_argument('--debug', action='store_true', default=False) # 指定是否开启调试模式，默认值为False
    parser.add_argument('--config', type=str) # 指定配置文件的路径
    parser.add_argument('--gpu', nargs='+', type=int) # 指定要使用的GPU设备的ID

    parser.add_argument('--node_rank', type=int, default=0) # 当前节点的排名
    parser.add_argument('--nodes', type=int, default=1) # 指定节点的总数，默认值为1
    parser.add_argument('--addr', type=str, default='127.0.0.1') # 制定节点之间通信的地址，默认值为'127.0.0.1'
    parser.add_argument('--port', type=int, default=11233) # 指定节点之间通信的端口，默认值为11233
 
    parser.add_argument('--signature', nargs='+', type=str) # 验证的签名
    parser.add_argument('--seed', type=int) # 随机数

    parser.add_argument('--eval', type=str) # 评估模型文件路径
    parser.add_argument('--eval_subdir', type=str) # 评估结果目录
    parser.add_argument('--pretrained', type=str) # 预训练模型的目录

    parser.add_argument('--resume_dir', type=str) # 指定要恢复训练的模型文件所在的目录
    parser.add_argument('--resume_step', type=int) # 指定要恢复训练的模型的步数
    parser.add_argument('--resume_weight', type=str) # 恢复训练的模型的权重文件路径

    args = parser.parse_args()

    # Special handling the resume 用于恢复训练
