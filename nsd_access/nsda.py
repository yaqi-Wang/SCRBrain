...
class NSDAccess(object):
    ...
    def read_atlas_results(self, subject, atlas='HCP_MMP1', data_format='fsaverage'):
        ...
        '''
        nsddata/freesurfer/
        ├── fsaverage/       # FreeSurfer 的标准化平均大脑模型
        ├── fsnative/        # 受试者个体的 FreeSurfer 处理结果
        ├── subjects/        # 存储多个受试者的大脑处理数据
        ├── label/           # 皮层分区标签 (Parcellation Labels)
        ├── surf/            # 皮层表面数据 (Cortical Surface)
        ├── mri/             # 体积数据 (MRI images)
        '''
